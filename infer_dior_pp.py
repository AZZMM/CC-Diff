...
from ccdiff_pp.utils import seed_everything
...
import inspect
from ccdiff_pp.attention_processor import set_processors
from ccdiff_pp.utils import dict_of_images, find_nearest, get_similar_examplers
from ccdiff_pp.projection import CoSampler
from ccdiff_pp.modules import FrozenDinoV2Encoder
...
class StableDiffusionMIPipeline(StableDiffusionPipeline):
    def __init__(
        ...
        self.image_proj_model = CoSampler(
            dim=1280,
            depth=4,
            dim_head=64,
            # heads=20,
    ...
    def __call__(
        ...
        ref_imgs = []
        for index, (caption, bbox) in enumerate(zip(prompt, bboxes[0])):
        ...
        with torch.no_grad():
            ...
            bg_img = self.feature_extractor(images=bg_img, return_tensors="pt")['pixel_values'].to(img_features.device)
            bg_features = self.image_encoder(bg_img)
        img_features, bg_features, rel_features = self.image_proj_model(img_features, obboxes, bg_features).split([ref_imgs.shape[0], 1, 1], dim=0)
        # bg_features = self.bg_proj_model(bg_features)
            ...
            for i, t in enumerate(timesteps):
                ...
                cross_attention_kwargs = {'bboxes': bboxes,
                                        ...
                                        #   'ref_features': ref_features,
                                          'ref_features': (img_features, bg_features, rel_features),
...
if __name__ == '__main__':
    distrbuted_state = PartialState()

    # sd1x_path = 'stable-diffusion-v1-4' if os.path.isdir('stable-diffusion-v1-4') else "CompVis/stable-diffusion-v1-4"
    ...
    pipe = pipe.to(distrbuted_state.device)
    ...
    num_process = 8
    chunks = [data[i:i + num_process] for i in range(0, len(data), num_process)]
    for sub_data in tqdm(chunks):
        with distrbuted_state.split_between_processes(sub_data) as sample:
            sample = sample[0]
        ...
        image.save(f'DIOR_result/{file_name}')
        # print()
        # image.show()
